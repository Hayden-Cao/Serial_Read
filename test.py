            ...
            with open("voltage_data.txt", "r") as file:
                for i, line in enumerate(file):
                    try:
                        ...
                        pd.DataFrame([voltage], columns=["Voltage (V)"]).to_excel(writer, index=False, header=False, startrow=i+1)
                    except ValueError:
        ...
        with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
            ...
            with open("voltage_data.txt", "r") as file:
                for i, line in enumerate(file):
                    try:
                        voltage = float(line.strip())
                        pd.DataFrame([voltage], columns=["Voltage (V)"]).to_excel(writer, index=True, header=False, startrow=i+1)
                    except ValueError:
                        display_message(f"Skipping invalid line {i + 1}: {line.strip()}")
        display_message(f"Data exported to {file_path}")
    except Exception as e:
